...
const validatePresentationConfig = async (schema) => {
  const config = normalizeSchema(schema).config || {};
  const viewIds = Array.isArray(config.view_ids) ? config.view_ids : [];
  if (viewIds.some(id => !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(id)))) return 'Presentations must reference view IDs';
  if (new Set(viewIds).size !== viewIds.length) return 'Presentation views cannot be duplicated';
  if (config.active_view_id && !viewIds.includes(config.active_view_id)) return 'Active view must be one of the selected presentation views';
  if (!viewIds.length) return null;
  const available = await pool.query('SELECT id FROM type_definitions WHERE kind=$1 AND id=ANY($2::uuid[])',['view',viewIds]);
  if (available.rowCount !== viewIds.length) return 'Presentation contains a missing or unavailable view';
  return null;
};
...
app.put('/api/types/:id', async (req,res) => { const {name,description='',schema={}}=req.body||{}; const normalized=normalizeSchema(schema); const schemaError=validateSchema(normalized); if(schemaError)return res.status(400).json({error:schemaError}); try { const current=await pool.query('SELECT kind FROM type_definitions WHERE id=$1',[req.params.id]); if(!current.rowCount)return res.status(404).json({error:'Type not found'}); const configError=current.rows[0].kind==='presentation'?await validatePresentationConfig(normalized):null; if(configError)return res.status(400).json({error:configError}); const r=await pool.query('UPDATE type_definitions SET name=COALESCE($1,name),description=$2,schema=$3 WHERE id=$4 RETURNING *',[name?.trim()||null,description,normalized,req.params.id]); res.json(r.rows[0]); } catch(e) { res.status(e.code==='23505'?409:500).json({error:e.message}); } });
app.delete('/api/types/:id', async (req,res) => { try { const current=await pool.query('SELECT kind,name FROM type_definitions WHERE id=$1',[req.params.id]); if(!current.rowCount)return res.status(404).json({error:'Type not found'}); if((current.rows[0].kind==='entity'&&current.rows[0].name==='Entity')||(current.rows[0].kind==='relationship'&&current.rows[0].name==='Relationship')||(current.rows[0].kind==='view'&&current.rows[0].name==='View')||(current.rows[0].kind==='presentation'&&current.rows[0].name==='Presentation'))return res.status(400).json({error:'Core definitions cannot be deleted'}); if(current.rows[0].kind==='view'){const used=await pool.query("SELECT id FROM type_definitions WHERE kind='presentation' AND schema->'config'->'view_ids' ? $1",[req.params.id]);if(used.rowCount)return res.status(409).json({error:'View is still selected by a presentation'});} await pool.query('DELETE FROM type_definitions WHERE id=$1',[req.params.id]); res.status(204).end(); } catch(e) { res.status(500).json({error:e.message}); } });
